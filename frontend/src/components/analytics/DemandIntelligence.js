...
const SEGMENT_COLORS = ["#007AFF", "#34C759", "#FF9500", "#AF52DE", "#32ADE6"];
...
const DemandIntelligence = () => {
  ...
  if (loading || !data) {
    return <div className="flex items-center justify-center" style={{ minHeight: 400 }}><p className="text-sm" style={{ color: "#A3A3A3" }}>Loading demand analytics...</p></div>;
  }
  ...
  return (
    <div className="space-y-5" data-testid="demand-analytics">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Current Demand", value: `${summary.current_demand} L/min`, color: "#007AFF" },
          ...
          <div key={card.label} className="p-3 rounded-sm border" style={{ backgroundColor: "#1A1A1A", borderColor: `${card.color}33` }}>
            <p className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: "#A3A3A3" }}>{card.label}</p>
      ...
      <div className="rounded-sm border p-4" style={{ backgroundColor: "#0D0D0D", borderColor: "rgba(255,255,255,0.08)" }}>
        <SectionHeader icon={<TrendUp size={16} color="#007AFF" weight="duotone" />} title="24H Demand Forecast (Actual vs Predicted)" color="#A3A3A3" />
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={forecast}>
            ...
            <Area type="monotone" dataKey="upper_bound" stroke="none" fill="#007AFF11" />
            <Area type="monotone" dataKey="lower_bound" stroke="none" fill="#007AFF11" />
            <Line type="monotone" dataKey="predicted" stroke="#007AFF" strokeWidth={2} dot={false} name="Predicted" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Peak Demand Heatmap */}
        <div className="rounded-sm border p-4" style={{ backgroundColor: "#0D0D0D", borderColor: "rgba(255,255,255,0.08)" }}>
          <SectionHeader icon={<ChartBar size={16} color="#FF9500" weight="duotone" />} title="Peak Demand Heatmap" color="#A3A3A3" />
          <div className="overflow-x-auto">
            <div className="flex items-center gap-0.5 mb-1">
              <div style={{ width: 28 }} />
              {Array.from({ length: 24 }, (_, h) => (
                <div key={h} className="text-center" style={{ width: 16, fontSize: 7, color: "#525252" }}>
                  {h % 4 === 0 ? `${h}` : ""}
                </div>
              ))}
            </div>
            {days.map((day) => (
              <div key={day} className="flex items-center gap-0.5 mb-0.5">
                <span style={{ width: 28, fontSize: 8, color: "#A3A3A3" }}>{day}</span>
                {Array.from({ length: 24 }, (_, h) => {
        ...
        <div className="rounded-sm border p-4" style={{ backgroundColor: "#0D0D0D", borderColor: "rgba(255,255,255,0.08)" }}>
          <SectionHeader icon={<Users size={16} color="#AF52DE" weight="duotone" />} title="Consumer Segmentation" color="#A3A3A3" />
              ...
              {segments.map((seg, i) => (
                <div key={seg.name} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px]" style={{ color: "#A3A3A3" }}>{seg.name}</span>
                  </div>
      ...
      {/* Seasonal Trend */}
      <div className="rounded-sm border p-4" style={{ backgroundColor: "#0D0D0D", borderColor: "rgba(255,255,255,0.08)" }}>
        <SectionHeader icon={<Thermometer size={16} color="#FF9500" weight="duotone" />} title="Seasonal Usage & Temperature Trend" color="#A3A3A3" />
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={seasonal_trend}>
            ...
            <Tooltip contentStyle={{ backgroundColor: "#1A1A1A", border: "1px solid #333", fontSize: 11, color: "#FFF" }} />
            <Legend wrapperStyle={{ fontSize: 10 }} />
            <Bar yAxisId="left" dataKey="avg_demand" name="Avg Demand (L/min)" fill="#007AFF" radius={[2, 2, 0, 0]} />
            <Bar yAxisId="left" dataKey="peak_demand" name="Peak Demand" fill="#007AFF44" radius={[2, 2, 0, 0]} />
