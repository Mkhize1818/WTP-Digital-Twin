...
const SEVERITY_COLORS = { critical: "#FF3B30", warning: "#FF9500", normal: "#34C759", info: "#32ADE6" };
...
const NRWAnalytics = () => {
  ...
  if (loading || !data) {
    return <div className="flex items-center justify-center" style={{ minHeight: 400 }}><p className="text-sm" style={{ color: "#A3A3A3" }}>Loading NRW analytics...</p></div>;
  }
  ...
  return (
    <div className="space-y-5" data-testid="nrw-analytics">
        ...
        <div className="space-y-2">
          {anomaly_flags.map((flag, i) => (
            <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-sm border" style={{ borderColor: SEVERITY_COLORS[flag.severity] + "55", backgroundColor: SEVERITY_COLORS[flag.severity] + "0D" }}>
              ...
              <span className="text-xs" style={{ color: "#A3A3A3" }}>{flag.message}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="rounded-sm border p-4" style={{ backgroundColor: "#0D0D0D", borderColor: "rgba(255,255,255,0.08)" }}>
        <SectionHeader icon={<ChartLine size={16} color="#007AFF" weight="duotone" />} title="24H Water Balance: Input vs Consumption vs Losses" color="#A3A3A3" />
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={hourly_balance}>
            <CartesianGrid strokeDasharray="3 3" stroke="#252525" />
            <XAxis dataKey="hour" tick={{ fontSize: 10, fill: "#525252" }} interval={3} />
            ...
            <Area type="monotone" dataKey="input" name="Input" stroke="#007AFF" fill="#007AFF22" strokeWidth={2} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* DMA Analysis */}
        <div className="rounded-sm border p-4" style={{ backgroundColor: "#0D0D0D", borderColor: "rgba(255,255,255,0.08)" }}>
          <SectionHeader icon={<Drop size={16} color="#32ADE6" weight="duotone" />} title="District Metered Area (DMA) Analysis" color="#A3A3A3" />
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {dma_analysis.map((dma) => (
              <div key={dma.id} className="flex items-center justify-between p-2 rounded-sm" style={{ backgroundColor: "#121212" }}>
                <div>
                  <p className="text-xs font-bold text-white">{dma.name}</p>
                  <p className="text-[10px]" style={{ color: "#525252" }}>{dma.pipe_km} km | {dma.material} | {dma.age_years}yr</p>
                </div>
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <p className="text-xs font-bold" style={{ color: SEVERITY_COLORS[dma.severity] }}>{dma.loss_pct}%</p>
                    <p className="text-[10px]" style={{ color: "#525252" }}>{dma.loss_volume} L/min</p>
                  </div>
        ...
        {/* MNF Profile */}
        <div className="rounded-sm border p-4" style={{ backgroundColor: "#0D0D0D", borderColor: "rgba(255,255,255,0.08)" }}>
          <SectionHeader icon={<Gauge size={16} color="#AF52DE" weight="duotone" />} title={`Minimum Night Flow (Baseline: ${mnf_baseline} L/min)`} color="#A3A3A3" />
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={mnf_profile}>
              <CartesianGrid strokeDasharray="3 3" stroke="#252525" />
              <XAxis dataKey="hour" tick={{ fontSize: 9, fill: "#525252" }} interval={3} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Loss Separation */}
        <div className="rounded-sm border p-4" style={{ backgroundColor: "#0D0D0D", borderColor: "rgba(255,255,255,0.08)" }}>
          <SectionHeader icon={<Warning size={16} color="#FF9500" weight="duotone" />} title="Apparent vs Real Loss Separation" color="#A3A3A3" />
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-2 flex-1">
              <div className="flex items-center justify-between p-2 rounded-sm" style={{ backgroundColor: "#121212" }}>
              ...
              <div className="pl-3 space-y-1">
                <div className="flex justify-between text-[10px]"><span style={{ color: "#A3A3A3" }}>Meter inaccuracy</span><span style={{ color: "#FF9500" }}>{loss_separation.apparent.meter_inaccuracy}</span></div>
                <div className="flex justify-between text-[10px]"><span style={{ color: "#A3A3A3" }}>Unauthorized</span><span style={{ color: "#FFD60A" }}>{loss_separation.apparent.unauthorized}</span></div>
              </div>
              <div className="flex items-center justify-between p-2 rounded-sm" style={{ backgroundColor: "#121212" }}>
              ...
              <div className="pl-3 space-y-1">
                <div className="flex justify-between text-[10px]"><span style={{ color: "#A3A3A3" }}>Pipe leaks</span><span style={{ color: "#FF3B30" }}>{loss_separation.real.pipe_leaks}</span></div>
                <div className="flex justify-between text-[10px]"><span style={{ color: "#A3A3A3" }}>Overflow</span><span style={{ color: "#FF6961" }}>{loss_separation.real.overflow}</span></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-sm border p-4" style={{ backgroundColor: "#0D0D0D", borderColor: "rgba(255,255,255,0.08)" }}>
          <SectionHeader icon={<Drop size={16} color="#FF3B30" weight="duotone" />} title="Leak Probability per Pipe Segment" color="#A3A3A3" />
          <div className="space-y-1.5 max-h-64 overflow-y-auto">
            {pipe_risk.sort((a, b) => b.leak_probability - a.leak_probability).map((pipe) => (
              <div key={pipe.id} className="flex items-center gap-3 p-2 rounded-sm" style={{ backgroundColor: "#121212" }}>
                <div className="flex-1">
                  <p className="text-[10px] font-bold text-white">{pipe.name}</p>
                  <p className="text-[9px]" style={{ color: "#525252" }}>{pipe.material} | {pipe.diameter_mm}mm | {pipe.age_years}yr</p>
                </div>
