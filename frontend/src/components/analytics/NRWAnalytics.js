...
const NRWAnalytics = () => {
  ...
  if (loading || !data) {
    return <div className="flex items-center justify-center" style={{ minHeight: 400 }}><p className="text-sm" style={{ color: "#163F56" }}>Loading NRW analytics...</p></div>;
  }
        ...
        <div className="space-y-2">
          {anomaly_flags.map((flag, i) => (
            <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-sm border" style={{ borderColor: SEVERITY_COLORS[flag.severity] + "55", backgroundColor: SEVERITY_COLORS[flag.severity] + "0D" }}>
              ...
              <span className="text-xs" style={{ color: "#163F56" }}>{flag.message}</span>
            </div>
          ))}
        </div>
      )}
            ...
            {dma_analysis.map((dma) => (
              <div key={dma.id} className="flex items-center justify-between p-2 rounded-sm" style={{ backgroundColor: "rgba(255,255,255,0.88)" }}>
                <div>
                  ...
                  <p className="text-[10px]" style={{ color: "#7A9AB5" }}>{dma.pipe_km} km | {dma.material} | {dma.age_years}yr</p>
                </div>
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <p className="text-xs font-bold" style={{ color: SEVERITY_COLORS[dma.severity] }}>{dma.loss_pct}%</p>
                    <p className="text-[10px]" style={{ color: "#7A9AB5" }}>{dma.loss_volume} L/min</p>
                  </div>
        ...
        <div className="rounded-sm border p-4" style={{ backgroundColor: "#F4F8FC", borderColor: "rgba(17,113,184,0.1)" }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={mnf_profile}>
              <CartesianGrid strokeDasharray="3 3" stroke="#252525" />
              <XAxis dataKey="hour" tick={{ fontSize: 9, fill: "#163F56" }} interval={3} />
              <YAxis tick={{ fontSize: 9, fill: "#163F56" }} />
              <Tooltip contentStyle={{ backgroundColor: "#FFFFFF", border: "1px solid rgba(17,113,184,0.2)", fontSize: 11, color: "#062C60" }} />
        ...
        <div className="rounded-sm border p-4" style={{ backgroundColor: "#F4F8FC", borderColor: "rgba(17,113,184,0.1)" }}>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-2 flex-1">
              <div className="flex items-center justify-between p-2 rounded-sm" style={{ backgroundColor: "rgba(255,255,255,0.88)" }}>
              ...
              <div className="pl-3 space-y-1">
                <div className="flex justify-between text-[10px]"><span style={{ color: "#163F56" }}>Meter inaccuracy</span><span style={{ color: "#FF9500" }}>{loss_separation.apparent.meter_inaccuracy}</span></div>
                <div className="flex justify-between text-[10px]"><span style={{ color: "#163F56" }}>Unauthorized</span><span style={{ color: "#FFD60A" }}>{loss_separation.apparent.unauthorized}</span></div>
              </div>
              <div className="flex items-center justify-between p-2 rounded-sm" style={{ backgroundColor: "rgba(255,255,255,0.88)" }}>
              ...
              <div className="pl-3 space-y-1">
                <div className="flex justify-between text-[10px]"><span style={{ color: "#163F56" }}>Pipe leaks</span><span style={{ color: "#FF3B30" }}>{loss_separation.real.pipe_leaks}</span></div>
                <div className="flex justify-between text-[10px]"><span style={{ color: "#163F56" }}>Overflow</span><span style={{ color: "#FF6961" }}>{loss_separation.real.overflow}</span></div>
              </div>
            </div>
          </div>
        </div>
            ...
            {pipe_risk.sort((a, b) => b.leak_probability - a.leak_probability).map((pipe) => (
              <div key={pipe.id} className="flex items-center gap-3 p-2 rounded-sm" style={{ backgroundColor: "rgba(255,255,255,0.88)" }}>
                <div className="flex-1">
                  ...
                  <p className="text-[9px]" style={{ color: "#7A9AB5" }}>{pipe.material} | {pipe.diameter_mm}mm | {pipe.age_years}yr</p>
                </div>
