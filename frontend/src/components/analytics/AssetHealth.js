...
const GRADE_COLORS = { A: "#34C759", B: "#32ADE6", C: "#FF9500", D: "#FF3B30" };
...
const AssetHealth = () => {
  ...
  if (loading || !data) {
    return <div className="flex items-center justify-center" style={{ minHeight: 400 }}><p className="text-sm" style={{ color: "#A3A3A3" }}>Loading asset health...</p></div>;
  }
    ...
    <div className="space-y-5" data-testid="asset-health-analytics">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Avg RUL", value: `${summary.avg_rul_years}`, unit: "years", color: "#32ADE6" },
          ...
          <div key={card.label} className="p-3 rounded-sm border" style={{ backgroundColor: "#1A1A1A", borderColor: `${card.color}33` }}>
            ...
            <div className="flex items-baseline gap-1">
              ...
              <span className="text-[10px]" style={{ color: "#525252" }}>{card.unit}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-sm border p-4" style={{ backgroundColor: "#0D0D0D", borderColor: "rgba(255,255,255,0.08)" }}>
        <SectionHeader icon={<Heartbeat size={16} color="#34C759" weight="duotone" />} title="Asset Condition & Failure Probability" color="#A3A3A3" />
              ...
              {assets.sort((a, b) => b.failure_probability - a.failure_probability).map((asset) => (
                <tr key={asset.id} className="border-t" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
                  <td className="py-2 px-2">
                    ...
                    <p style={{ color: "#525252" }}>{asset.length_m}m | {asset.diameter_mm}mm</p>
                  </td>
                  <td className="py-2 px-2" style={{ color: "#A3A3A3" }}>{asset.material}</td>
                  <td className="text-center py-2 px-2" style={{ color: "#A3A3A3" }}>{asset.age_years}yr</td>
                  ...
                  <td className="text-center py-2 px-2" style={{ color: "#32ADE6" }}>{asset.rul_years}yr</td>
                  <td className="text-center py-2 px-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Break History */}
        <div className="rounded-sm border p-4" style={{ backgroundColor: "#0D0D0D", borderColor: "rgba(255,255,255,0.08)" }}>
          <SectionHeader icon={<HardDrives size={16} color="#AF52DE" weight="duotone" />} title="Break History (12 Months)" color="#A3A3A3" />
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={break_history}>
              <CartesianGrid strokeDasharray="3 3" stroke="#252525" />
              <XAxis dataKey="month" tick={{ fontSize: 9, fill: "#525252" }} angle={-45} textAnchor="end" height={50} />
              <YAxis tick={{ fontSize: 9, fill: "#525252" }} allowDecimals={false} />
        ...
        <div className="rounded-sm border p-4" style={{ backgroundColor: "#0D0D0D", borderColor: "rgba(255,255,255,0.08)" }}>
          <SectionHeader icon={<Wrench size={16} color="#007AFF" weight="duotone" />} title="Predictive Maintenance Schedule" color="#A3A3A3" />
          <div className="space-y-2">
            {maintenance_schedule.map((item) => (
              <div key={item.asset_id} className="p-2.5 rounded-sm border" style={{ backgroundColor: "#121212", borderColor: item.type === "urgent" ? "#FF3B3033" : "rgba(255,255,255,0.05)" }}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[9px] px-1.5 py-0.5 rounded-sm font-bold uppercase" style={{
                    backgroundColor: item.type === "urgent" ? "#FF3B3015" : "#007AFF15",
                    color: item.type === "urgent" ? "#FF3B30" : "#007AFF",
                  }}>
                    {item.type}
                  </span>
                </div>
                <div className="flex items-center justify-between text-[10px]">
                  ...
                  <span style={{ color: "#525252" }}>Est. R{item.estimated_cost.toLocaleString()}</span>
                </div>
              </div>
            ))}
          </div>
