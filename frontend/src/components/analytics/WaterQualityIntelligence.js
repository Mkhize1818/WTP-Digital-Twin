...
const WaterQualityIntelligence = () => {
  ...
  if (loading || !data) {
    return <div className="flex items-center justify-center" style={{ minHeight: 400 }}><p className="text-sm" style={{ color: "#163F56" }}>Loading water quality...</p></div>;
  }
    ...
    <div className="space-y-5" data-testid="water-quality-analytics">
      ...
      >
        ...
        <div className="flex items-center gap-4 text-[10px]">
          <span style={{ color: "#163F56" }}>Zones: {summary.compliant_zones}/{summary.total_zones}</span>
          <span style={{ color: "#163F56" }}>Params: {summary.parameters_in_spec}/{summary.total_parameters}</span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {quality_parameters.map((param) => (
          <div key={param.id} className="p-3 rounded-sm border" style={{ backgroundColor: "#FFFFFF", borderColor: param.in_spec ? "#34C75933" : "#FF3B3055" }}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: "#163F56" }}>{param.name}</span>
            ...
            <div className="flex items-baseline gap-1 mb-2">
              ...
              <span className="text-[10px]" style={{ color: "#7A9AB5" }}>{param.unit}</span>
            </div>
            {/* Range bar */}
            ...
            <div className="flex justify-between mt-1 text-[8px]" style={{ color: "#7A9AB5" }}>
        ...
        <div className="rounded-sm border p-4" style={{ backgroundColor: "#F4F8FC", borderColor: "rgba(17,113,184,0.1)" }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={decay_curve}>
              ...
              <XAxis dataKey="distance_m" tick={{ fontSize: 9, fill: "#163F56" }} label={{ value: "Distance (m)", position: "bottom", offset: -5, style: { fontSize: 9, fill: "#163F56" } }} />
              <YAxis tick={{ fontSize: 9, fill: "#163F56" }} domain={[0, "auto"]} />
            ...
            {contamination_events.map((event) => (
              <div key={event.id} className="p-2 rounded-sm flex items-start gap-2" style={{ backgroundColor: "rgba(255,255,255,0.88)" }}>
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[9px] mt-0.5" style={{ color: "#163F56" }}>{event.zone} | {event.duration_hours}h | {event.source_trace}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            {predictions.map((pred) => (
              <div key={pred.parameter} className="p-2.5 rounded-sm" style={{ backgroundColor: "rgba(255,255,255,0.88)" }}>
                ...
                <div className="flex items-center justify-between text-[10px]">
                  <span style={{ color: "#163F56" }}>Current: {pred.current_value}</span>
                  {pred.hours_to_breach && (
                    <span style={{ color: "#FF3B30" }}>Breach in ~{pred.hours_to_breach}h</span>
                  )}
                  <span style={{ color: "#7A9AB5" }}>Conf: {(pred.confidence * 100).toFixed(0)}%</span>
                </div>
              </div>
            ))}
          </div>
